...
- **Feat**: Added `--github-annotations` (abbr: `-g`) flag to output GitHub Actions warning annotations for uncovered lines.
- **Perf**: Optimized coverage reporting by caching coverage calculations in display loops.

## 0.7.0

- **Feat**: Native support for Dart VM coverage JSON files (`.json`) generated by `dart test --coverage`.
